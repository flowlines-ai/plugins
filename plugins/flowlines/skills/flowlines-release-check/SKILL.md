...
2. Establish the trend on both sides: `aggregate_sessions` for a range that covers both windows, `group_by: ["day", "status"]`, metrics `session_count`, `user_count`, `success_count`, `failure_count`, `success_rate`, `total_cost_usd`, `unanalyzed_count`, filtered by `agent_name`. Split the days at the boundary yourself and compute before and after averages. Sessions arriving in the after-window are still being analysed; a high `unanalyzed_count` makes the after-side rate provisional, so say so.
3. `get_metric_definition` for `success_rate` and any other rate you will report. Respect the sample floor: below it, report counts and say the rate is not yet meaningful.
4. Find what moved: `aggregate_sessions` with `group_by: ["intent"]` filtered by the agent for the after-window range, then the same for a range that mostly covers the before-window, and compare per intent. Intents that appear or disappear across the boundary are as important as rate changes.
