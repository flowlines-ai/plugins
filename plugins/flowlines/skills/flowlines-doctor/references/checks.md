...
Every step below that reads a page of the Flowlines app is for the user to perform: name the page and what to look for, and do not open the app, a browser, or a search engine yourself unless the user explicitly asks. Everything else runs from a shell or over the Flowlines MCP server.
...
3. Ask the user to open the MCP page of the Flowlines app and report its ingestion health; the MCP server does not expose it. The status is derived from a durable ledger of every MCP-shaped batch:
