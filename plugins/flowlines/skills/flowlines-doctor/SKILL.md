...
- If the error repeats, inspect the client's MCP startup status and available local logs before escalating to deployment logs. Match the Flowlines server, current failure time, and task or connection when available. In the Codex desktop app on macOS, app logs are under `~/Library/Logs/com.openai.codex/YYYY/MM/DD/`; use the current client's log location for other clients.
...
1. Use the failing client's dedicated MCP or plugin sign-in/reconnect action first. For a local Codex MCP connection, use `codex mcp login flowlines`; in Claude Code, invoke `/mcp`, select `flowlines`, and authenticate. A local CLI login does not repair a separate hosted connector; use that connector's reconnect action instead. Do not stop to ask which method to try when one of these is available.
2. Before a shell login, account for browser launch and access to the client's credential store, including macOS Keychain. If the command needs execution permission outside the sandbox to use them, use the client's normal approval mechanism for that command. Do not disable the sandbox globally, change credential storage settings, or read saved tokens.
3. If the authentication action returns an authorization URL but does not launch it, open that exact URL with the OS URL opener (`open` on macOS, `xdg-open` on Linux). Do not search for a login page, alter the URL, or paste an authorization URL containing state or codes into chat or the final report.
...
5. Confirm that the client saved the credentials. A browser callback followed by `failed to write OAuth tokens to keyring` or a secure-storage permission error is not a completed login. If this happens, correct the local execution permission first; allow one additional login attempt for this storage failure only. Do not loop through browser consent without resolving the storage error.

### Verify and resume

After credentials are saved, retry one read-only Flowlines call such as `get_workspace`. If it succeeds, resume the original request. A successful login alone does not prove MCP tool access is restored.

If the same connection failure persists, use an available native action to reload that MCP connection once, then make one final read-only check. If reload is unavailable or verification still fails, stop. State that credentials were saved but tool access remains unverified, name the error, and give the client-specific reconnect or restart step. Do not restart the whole app automatically or interrupt other work. A persistent `403` calls for checking account access to the workspace, not another login.
