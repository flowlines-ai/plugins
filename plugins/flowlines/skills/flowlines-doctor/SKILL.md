...
- Some facts live only in the Flowlines app: connector status, the paused-server toggle, identity mappings, and MCP ingestion health when the server does not offer `get_mcp_overview`. When a check needs one of them, read it in the app if a signed-in browser session is available, or ask the user to read it; say which you did. Do not open the app for anything the MCP server exposes, and do not search the web unless a fact is blocking the diagnosis.
...
- **An instrumented MCP server.** Confirm the OTLP environment variables are set in the deployment, run ten tool calls plus `report_outcome`, then verify arrival over MCP: `list_agents` for the server's service name and `list_sessions` with `from` set a few minutes back. Read the ingestion health status (five values) with `get_mcp_overview` (`range` `30d`); when the server does not offer that tool, read it on the MCP page of the Flowlines app or ask the user to. Say what each status implicates, from the reference. The MCP server does not expose per-tool failure counts. A paused server under Settings, MCP stops derived observability without stopping ingestion.
