...
- If authentication is required, or a generic MCP error (such as `-32603`) repeats on one read-only check, use `flowlines-doctor` when available for connection recovery before further data calls. Client diagnostics and native sign-in are allowed for this repair. After verification, resume the original request with the same namespace, range, and `user_intent`.
- If `flowlines-doctor` is unavailable, use only the failing client's connection status and native reconnect action. Reconnect once only when that client reports an authentication problem; leave browser consent to the user, then verify with one read-only call. If the action is unavailable, give the user the reconnect step in their client and wait for completion. For an unexplained generic error or failed verification, stop data calls, give the user the error and a pointer to Flowlines support. Do not access Flowlines infrastructure or present unavailable data as empty; state when `report_outcome` could not be sent.
