...
The same modal's **Observed** tab shows what agents actually send and receive, computed nightly from the captured arguments and results of tools with at least 20 calls carrying a session in the last 7 days: per path, the share of calls carrying it, the JSON types seen, and a note against the declared schema (missing when required, not declared, type differs, values outside the enum, never sent). Flowlines stores shape counts only, never payload values, except a tally of the values a declared `enum` lists. Sending `gen_ai.tool.input_schema` is therefore what turns the observed view from a listing into a drift check.
